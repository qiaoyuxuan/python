...
import time
from uuzu.app.com import operationFile,md5


def key_sort(body):
	'''
	根据key排序，按首字母从大到小，然后按规则拼接
	:param body: dict数据
	:return: 根据key首字母排序后的dict
	'''
	# print(kwargs.items())
	body_sort = sorted(body.items(), key=lambda x: x[0])  # 排序方法，返回根据key首字母排序后的dict
	# print('key_sort方法：', body_sort)
	return body_sort
...
def login_sign(mobile):
	...
	:return:返回加密后的sign
	'''
	get_yaml = operationFile.read_Yaml('app/file/login_sign.yaml')  # 读取待加密的数据
	get_yaml['ts'] = int(time.time())  # 修改待加密数据中ts为当前时间戳
	...
	appkey = None
	if mobile == 'ios':
		appkey = 'appkey=wpRDTS9g7S3usDtzC5t3'
	elif mobile == 'android':
		appkey = 'appkey=tMhjPHHzb8q7NGkWvXT8'

	# 拼接字符串和appkey
	string_join_app = string + appkey
	print(string_join_app)
